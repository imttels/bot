...
import db
from drive_client import get_drive_service, find_file_by_name, download_file, find_month_folder, list_pdfs_in_folder
    ...
    if success:
        ...
        if employee_chat_id in ADMIN_CHAT_IDS:
            await update.message.reply_text(
                f"Нельзя удалить '{name}' — это администратор."
            )
        else:
